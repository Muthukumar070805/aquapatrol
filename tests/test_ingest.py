...
@pytest.fixture(autouse=True)
def _clean_cdse_env(monkeypatch: pytest.MonkeyPatch) -> None:
    """Neutralise real CDSE credentials so tests never inherit a local ``.env``.

    ``CDSESettings`` reads ``.env`` from the working directory, and explicit
    environment variables take precedence over ``.env`` values — so blanking
    the four vars here beats both ``os.environ`` and the file. A developer
    machine with live credentials would otherwise flip grant selection and
    break hermetic tests. Tests needing env creds set them explicitly.
    """
    for var in ("CDSE_USER", "CDSE_PASS", "CDSE_CLIENT_ID", "CDSE_CLIENT_SECRET"):
...
def test_get_access_token_client_credentials() -> None:
    fixture = _load_fixture("token_response.json")
    session = _RecordingSession(post_response=_FakeResponse(json_data=fixture))
    ...
    assert call["data"]["client_id"] == "my-client-id"
    assert call["data"]["client_secret"] == "my-client-secret"
    assert "username" not in call["data"]
...
def test_cdse_settings_reads_client_credentials(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CDSE_CLIENT_ID", "my-client-id")
    monkeypatch.setenv("CDSE_CLIENT_SECRET", "my-client-secret")
    from oilspill.pipeline.ingest import CDSESettings

    settings = CDSESettings()
    assert settings.client_id == "my-client-id"
    assert settings.client_secret == "my-client-secret"
