    ...
    for tile in tiles:
        ...
        for r in results:
            ...
            for box_idx in range(len(boxes)):
                ...
                x1 = max(0, min(w, tile.x + math.floor(xyxy[0])))
                y1 = max(0, min(h, tile.y + math.floor(xyxy[1])))
                x2 = max(0, min(w, tile.x + math.ceil(xyxy[2])))
                y2 = max(0, min(h, tile.y + math.ceil(xyxy[3])))
                if x2 <= x1 or y2 <= y1:
                    continue
                all_dets.append(
                    RawDetection(
...
def extract_contour(
    ...
    *,
    ...
    _thresh_val, binary = cv2.threshold(norm, 0, 255, cv2.THRESH_BINARY_INV + cv2.THRESH_OTSU)
    ...
    if num_labels <= 1:
        return ContourResult(mask=None, success=False, quality_flags=["contour_extraction_failed"])
...
def compute_investigation_confidence(
    ...
    *,
    ...
    if wind_speed is not None:
        if wind_speed < low_wind_threshold:
            ...
            breakdown.append(
                ConfidenceAdjustment(
                    reason=(
                        f"Moderate+ wind ({wind_speed:.1f} m/s): dark patch less likely a calm zone"
                    ),
                    delta=bonus,
                )
            )
    else:
        breakdown.append(ConfidenceAdjustment(reason="Wind data unavailable", delta=0.0))
    ...
    if optical is True:
        ...
        breakdown.append(ConfidenceAdjustment(reason="Optical confirmation unavailable", delta=0.0))
