...
* Keycloak access token (``grant_type=client_credentials`` with a dashboard
  OAuth client, or ``grant_type=password``, ``client_id=cdse-public`` with an
  account login):
...
def get_access_token(
    ...
    *,
    ...
    :class:`ValueError` if no credentials are configured.
    """
    settings = CDSESettings()
    client_id = client_id if client_id is not None else settings.client_id
    client_secret = client_secret if client_secret is not None else settings.client_secret
    ...
    if user and password:
        ...
        raise ValueError(
            "CDSE credentials missing; set CDSE_CLIENT_ID and CDSE_CLIENT_SECRET "
            "(OAuth client) or CDSE_USER and CDSE_PASS (account login) "
