...
export default function Hindcast() {
  ...
  useEffect(() => {
    ...
    // Initialise once; early-return guards re-runs when lat/lon inputs change.
  }, [lat, lon]);
  ...
  useEffect(() => {
    if (map.current && result?.geojson) {
      ...
      if (src) {
        src.setData(result.geojson as unknown as GeoJSON.FeatureCollection);

        interface PointFeature {
          geometry: { type: string; coordinates: [number, number] };
        }
        const coords = (result.geojson.features as unknown as PointFeature[])
          .filter((f) => f.geometry.type === "Point")
          .map((f) => f.geometry.coordinates);
          
        if (coords.length > 0) {
          const bounds = coords.reduce((b: maplibregl.LngLatBounds, c: [number, number]) => {
  ...
  const executeSimulation = async () => {
    ...
    try {
      ...
      setError(err instanceof Error ? err.message : "Failed to execute hindcast simulation");
    } finally {
      setLoading(false);
    }
  };
          ...
          {result && (
            <div style={{ marginTop: "1.5rem" }}>
              ...
              <div className="row" style={{ marginTop: "1rem" }}>
                <Link
                  to={`/vessels?origin_lat=${result.json.probable_spill_origin.latitude.toFixed(4)}&origin_lon=${result.json.probable_spill_origin.longitude.toFixed(4)}`}
                >
                  Investigate Vessels →
                </Link>
              </div>
