...
import type { Job, YoloRawDetection } from "../lib/types";
...
export default function SceneMonitor() {
  ...
  // Render YOLO candidates when a job completes.
  useEffect(() => {
    const m = map.current;
    if (!m || !m.isStyleLoaded()) return;
  ...
  function downloadGeoJSON() {
    ...
    a.href = url;
    a.download = "oil-candidates.geojson";
    a.click();
  ...
  return (
    <section>
      <div className="view-head">
        <h1>Scene Monitor</h1>
        <p>
          Define an area of interest and a date range, then queue a YOLO scene
          analysis. Candidate spill boxes are drawn on the map.
        </p>
      </div>

      <div className="scene-grid">
        <div className="panel">
          {yoloStatus && !yoloStatus.available && (
            <div className="error-box" style={{ marginBottom: "1rem" }}>
              YOLO detector unavailable: {yoloStatus.detail}
            </div>
          )}
          ...
          <button
            className="primary"
            style={{ width: "100%" }}
            disabled={running || yoloStatus?.available === false}
            onClick={submit}
          >
            {running ? (
              <>
