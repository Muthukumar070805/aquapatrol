...
The YOLO detector is the project's sole inference pathway. It generates oil-candidate bounding boxes using an Ultralytics YOLO model (one-class `oil` checkpoint), served through the API and frontend.

## Detector Boundary

There is one detector path: `yolo_mvp`, an Ultralytics detector using an
...
*   `OILSPILL_API_YOLO_WEIGHTS`: Path to the YOLO checkpoint (default: `artifacts/yolo/best.pt`, the gitignored local copy; absolute paths also work). If the file is missing, the YOLO pathway is gracefully disabled.
